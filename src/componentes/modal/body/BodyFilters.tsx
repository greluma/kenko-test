...
export default function BodyFilters({
  ...
  return (
    <div className="flex my-2 sm:my-4 lg:my-5 gap-3 justify-center">
      <button
        onClick={() => handleSetEstado(0)}
        className={`py-0.5 rounded-md px-3 cursor-pointer text-center   max-w-fit text-sm ${
          estado === 0
      ...
      <button
        ...
        className={` py-0.5 cursor-pointer rounded-md px-3 text-center border  max-w-fit text-sm ${
          estado === 2
            ? 'border-slate-900 text-slate-900 border-2 font-semibold'
            : 'border-slate-500 text-slate-500 border font-normal'
        }`}
